...
def protein_init(Sequence): #,organism
    result_dict = {}
    model_location = "esm2_t33_650M_UR50D"
    model, alphabet = esm.pretrained.load_model_and_alphabet(model_location)
    model.eval()
    ...
    for seq in Sequence:
        if len(seq) > 1000:
            seq = seq[:1000]
        seq_re = seq.replace('U', 'X').replace('Z', 'X').replace('O', 'X').replace("B", "X")
        seqs[seq] = seq_re
    ...
    for seq in tqdm(Sequence):
        ...
        result_dict[seq] = {
            'seq':seq_max,
            'token_representation': torch.from_numpy(res_feature),
            'token_representation_esm': token_repr.half(),
